...
#READING IN THE DATA SETS - ALSO PRESENT IN DATA FOLDER
...
#FUNCTION TO TAKE ATTRIBUTE THE PERSONALITY TYPE OF EACH CHARACTER BASED ON WHICH THEY MATCH WITH THE MOST
...
#SUB GROUPING OF DATA BY ADDING TWO NEW COLUMNS FOR SOCIALISATION AND HOW THEY PERCEIVE THE WORLD
...
#LINKING OF THE MYERS_BRIGGS.CSV AND CHARACTERS.CSV TO JOIN PERSONALITY TYPES AND NOTABILITY SCORES - THEN ARRANGED BY UNIVERSE NAME IN ALPHABETICAL ORDER
...
#FOUR UNIVERSE PLOTS TO MAP THE 99 UNIVERSES COUNTS OF CHARACTERS OF GIVEN PERSONALITY TYPES
#EACH PLOT TAKES A SUBSECTION OF THE ALPHABETICALLY-ORDERED UNIVERSES AND APPLIES THE FOLLOWING ELEMENTS
#CREATES A RADIAL COLUMN PLOT OF MYERS-BRIGGS AGAINST NOTABILITY, FACET WRAPPED BY UNIVERSE NAME

myersBriggsUniversePlot1 <- ggplot(myers_briggs_linked[starts_with(LETTERS[1:6], 
                                    vars=myers_briggs_linked$uni_name),]) + 
                              theme_minimal() + 
                              geom_col(aes(x = myers_briggs, 
                                            y = notability, 
                                            fill = uni_name), 
                                        size=.05, 
                                        color="black", 
                              ...
                              labs(title = "The number of characters that hold the highest correlation to a respective Myers-Briggs Personality Type: ", subtitle="Per universe, from universes titled A - F", 
                              x = "Myers-Briggs Personality Type") + 
                              coord_polar() + 
                              facet_wrap2(vars(uni_name))


myersBriggsUniversePlot2 <- ggplot(myers_briggs_linked[starts_with(LETTERS[7:13], vars=myers_briggs_linked$uni_name),]) + 
                              theme_minimal() + 
                              geom_col(aes(x = myers_briggs, 
                                            y = notability, 
                                            fill = uni_name), 
                              ...
                              theme(axis.title.y = element_blank(), 
                                    ...
                                    strip.clip = "off") + 
                              scale_y_continuous(breaks = scales::breaks_width(20)) + 
                              labs(title="The number of characters that hold the highest correlation to a respective Myers-Briggs Personality Type: ", 
                              subtitle="Per universe, from universes titled G - M") + 
                              coord_polar() + 
                              facet_wrap2(vars(uni_name))


myersBriggsUniversePlot3 <- ggplot(myers_briggs_linked[starts_with(LETTERS[14:19], vars=myers_briggs_linked$uni_name),]) + 
                              theme_minimal() + 
                              geom_col(aes(x = myers_briggs, 
                                            y = notability, 
                                            fill = uni_name), 
                              ...
                              theme(axis.title.y = element_blank(), 
                                      ...
                                      strip.clip = "off") + 
                              scale_y_continuous(breaks = scales::breaks_width(20)) + 
                              labs(title="The number of characters that hold the highest correlation to a respective Myers-Briggs Personality Type: ", subtitle="Per universe, from universes titled N - S") + 
                              coord_polar() + 
                              facet_wrap2(vars(uni_name))


myersBriggsUniversePlot4 <- ggplot(myers_briggs_linked[starts_with(LETTERS[20:26], vars = myers_briggs_linked$uni_name),]) +
                              theme_minimal() + 
                              geom_col(aes(x = myers_briggs, 
                                            y = notability, 
                                            fill = uni_name), 
                              ...
                              theme(axis.title.y = element_blank(), 
                                ...
                                scale_y_continuous(breaks = scales::breaks_width(20)) + 
                                labs(title = "The number of characters that hold the highest correlation to a respective Myers-Briggs Personality Type: ", 
                                      subtitle="Per universe, from universes titled T - Z") + 
                                coord_polar() + 
                                facet_wrap2(vars(uni_name))
...
notabilityMeans_TheWalkingDead <- ggplot(filter(myers_briggs_linked, uni_name == "The Walking Dead"), aes(group = myers_briggs, x = myers_briggs)) + 
                                    theme_minimal() + 
                                    ...
                                    theme(plot.background = element_rect(color = "white"),  
                                            ...
                                            panel.grid.major.x = element_blank(), 
                                            legend.position = "none") + 
                                    scale_y_continuous(breaks = scales::breaks_width(10), expand = expand_scale(c(0,0), c(0,35))) + 
                                    labs(title="Plot of the number of The Walking Dead characters that\nare predominantly of a chosen personality type: ", 
                                          subtitle="with mean notability score for said personality type highlighted", 
                                          x = "Myers Briggs Personality Type", 
                                          y = "Notability Score") +
                                    geom_vline(xintercept = seq(0.5, after_stat(nrow(myers_briggs)), 1),
                                                  color="#FFAAFF", 
                                                  alpha = .5) + 
                                    stat_summary(geom="text",       
                                                  fun = "mean", 
                                                  aes(label = floor(after_stat(y)), 
                                                  x = myers_briggs, 
                                                  y = notability), 
                                                  size = 2, 
                                                  vjust = -2) + 
                                    geom_from_path(aes(x = myers_briggs, 
                                                        y = 95, 
                                                        path = image_link, 
                                                        group = myers_briggs), 
                                                        width = .2, 
...
notabilityScores_GameOfThrones <- ggplot(myers_briggs_linked[myers_briggs_linked$uni_name %in% c("Game of Thrones"),], aes()) + 
                                  theme_minimal() + 
                                  geom_jitter(aes(x = myers_briggs, 
                                                  y = notability, 
                                                  pch = socialisation), 
                                  ...
                                  panel.grid.major.x = element_blank()) + 
                                  scale_y_continuous(breaks = scales::breaks_width(10)) + 
                                  labs(title = "Plot of the number of Game of Thrones characters that\nare predominantly of a chosen personality type: ", 
                                        subtitle = "with percentage match to said personality type highlighted", 
                                        x = "Myers Briggs Personality Type", 
                                        y = "Notability Score", 
                                        caption = "pm = Percentage match for that character", pch = "Socialisation") + 
                                  geom_text_repel(aes(label = paste("pm=",avg_match_perc), 
                                                      x = myers_briggs, 
                                                      y = notability), 
                                                  size = 3, 
                                                  vjust = -1, 
...
notabilityMeans_GameOfThrones <- ggplot(filter(myers_briggs_linked, uni_name == "Game of Thrones"), aes(group = myers_briggs, x = myers_briggs)) + 
                                    geom_bar(aes( y = notability, fill = myers_briggs), width=.4, stat = "summary", fun = "mean", colour = "black", size = 0.2) + 
                                    theme_minimal() + 
                                    theme(plot.background = element_rect(color = "white"),  
                                            panel.grid.minor.y = element_blank(), 
                                            title = element_text(size = 11), 
                                            ...
                                            panel.grid.major.x = element_blank(), 
                                            legend.position = "none") + 
                                    scale_y_continuous(breaks = scales::breaks_width(10), expand = expand_scale(c(0,0), c(0,20))) + 
                                    labs(title = "Plot of the number of Game of Thrones characters that\nare predominantly of a chosen personality type: ", 
                                          subtitle = "with mean notability score for said personality type highlighted", 
                                          x = "Myers Briggs Personality Type", 
                                          y = "Notability Score") +
...
perNetwork_MeanNotability <- ggplot(myers_briggs_grouped_notability) + 
                              geom_bar(aes(x = myers_briggs, fill = Network, y = notability), stat="summary", fun = "mean", position = position_dodge(.9), width = .5, colour="black", size = 0.2) + 
                              theme_minimal() + 
                              theme(plot.background = element_rect(color = "white"), 
                                      panel.grid.minor.y = element_blank(), 
                                      title = element_text(size = 11), 
                                      ...
                                      panel.grid.major.x = element_blank()) + 
                              scale_y_continuous(breaks = scales::breaks_width(10), 
                              expand = expand_scale(c(0,0), c(0,4))) + 
                              labs(title="Mean of the Myers-Briggs personality types by Network", 
                                    subtitle="From every universe", 
                                    x = "Myers Briggs Personality Type", 
                                    y = "Mean notability") + 
                              facet_wrap2(vars(socialisation), scales = "free_x", nrow = 2) + scale_fill_brewer(palette = "YlOrRd") + 
...
```{r}
#| label: Question-Two-Preparation
...
# Frequency distribution of character grouped personality traits
question2Part1Plot1 = ggplot(personalityDistribution, 
                            aes(x = factor(
                      fct_rev(fct_inorder(personality_category))), y = n)) +
                        geom_bar(stat = "identity", aes(fill=(personality_category)), color = "black") + scale_fill_brewer(palette = "Reds") + labs(title = "Frequency distribution of character grouped personality traits",
                            x = "Grouped Personality Traits",
                            y = "Frequency")  +
                        coord_flip() +
                        theme_minimal()+
...
# "Average rating by character grouped personality traits
question2Part1Plot2 = ggplot(avgRating, aes(x = factor(
                      ...
                      geom_bar(stat = "identity", 
                              ...
                              color = "black") +
                      labs(title = "Average rating by character grouped personality traits",
                          x = "Grouped Personality Traits",
                          y = "Average Rating") +
                      coord_flip() +
                      theme_minimal() + 
...
# Density ridges plot of grouped personality traits
...
# Average notability by character grouped personality traits
question2Part2Plot = ggplot(avgNotability, aes(x = factor(
                      ...
                      aes(fill= (personality_category)), 
                      color = "black") +
                      labs(title = "Average notability by character grouped personality traits",
                          x = "Grouped Personality Traits",
...
Character personality trait distribution, average ratings, and notability scores are interestingly shown by analyzing the attributes over multiple universes. The frequency distribution of characters within each trait group does not seem to significantly correspond with their average rating, despite the variety of personality types. This implies that a character's success and attraction are influenced by factors other than just their personality.

Furthermore, the research shows that only the "balanced" and "other" traits exhibit a right skew, with the distribution of characters across personality traits generally being zero-skewed. This implies that some personality groups might be more common or favored in narrative, maybe because of their relatability or adaptability to different tale situations.

Nevertheless, the statistics show negligible variations when notability ratings are compared throughout personality categories. This suggests that a character's personality qualities could not have a big influence on how important or prominent they are seen to be in a story. Character notability may instead be more heavily influenced by elements including character arc, role importance, and impact on the plot.

All things considered, personality qualities add to the complexity of characters and the depth of the story, but they may have a less impact on audience reaction and character prominence than other elements. Subsequent investigations may explore the intricate relationship among personality characteristics, character progression, and audience perception to acquire a more all-encompassing comprehension of the factors influencing character triumph in narrative.
